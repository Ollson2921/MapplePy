...
class MTRowColSeperation:
    ...
    def create_new_param_maps(self,param : Parameter,direction):
        ...
        for item in self.preimage_map[direction].items():
            ...
            if len(item[1]) > 1:
                additions += len(preimages)
                for index in preimages:
                    new_map[index + additions] = item[1][1]
                    temp_map[index + additions] = index
    ...
    def adjust_parameter(self, param : Parameter):
        ...
        temp_parameter = Parameter(Tiling([],[],(len(new_col_map),len(new_row_map))),RowColMap(temp_col_map,temp_row_map)).back_map_obs_and_reqs(param.ghost)
    ...
    def separate_base_tiling(self):
        ...
        new_avoiders = [self.adjust_parameter(param) for param in self.avoiding_parameters]
        new_containers = []
        for c_list in self.containing_parameters:
            new_c_list = [self.adjust_parameter(param) for param in self.containing_parameters]
            new_containers.append(new_c_list)
        new_enumerators = []
        for e_list in self.enumeration_parameters:
            new_e_list = [self.adjust_parameter(param) for param in self.enumeration_parameters]
            new_enumerators.append(new_e_list)
        seperated = self.separation.row_col_separation()
        for T in seperated:
            yield MappedTiling(T,new_avoiders,new_containers, new_enumerators).full_cleanup() #Full clean up is probably overkill, we primarily just need backmapping
    ...
    def separate_parameter(param : Parameter):
        ...
        new_ghost = separation.row_col_separation()
        separation_map = separation.row_col_map
        new_col_map = {i : param.map.col_map[separation_map.col_map[i]] for i in separation_map.col_map}
        new_row_map = {i : param.map.row_map[separation_map.row_map[i]] for i in separation_map.row_map}
