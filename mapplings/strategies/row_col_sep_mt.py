...
class LTRowColSeparationMT:
    ...
    def cell_map(self, total_extensions, param: Parameter):
        """Maps each cell of the original parameter to the correct active cell in the original parameter
        also tracks which cells will be empty"""
        cell_map = {}
        ...
        for base_cell in cells_to_move:
            if base_cell in param.image_cells():
                for cell in param.map.preimage_of_cell(base_cell):
                    cell_map[cell] = (
                        cell_map[cell][0] + total_extensions[0][cell[0] + 1],
                        cell_map[cell][1],
                    )
        ...
        for base_cell in cells_to_move:
            if base_cell in param.image_cells():
                for cell in param.map.preimage_of_cell(base_cell):
                    cell_map[cell] = (
                        cell_map[cell][0],
                        cell_map[cell][1] + total_extensions[1][cell[1] + 1],
                    )
        return cell_map
...
class MTRowColSeparation:
    ...
    def cell_map_and_empty_cells(self, total_extensions, param: Parameter):
        """Maps each cell of the original parameter to the correct active cell in the original parameter
        also tracks which cells will be empty"""
        cell_map = {}
        empty_cells = set()
        ...
        inequalities = self.separation.column_row_inequalities()
        for pair in inequalities[0]:
            for cell in param.map.preimage_of_cell(pair[1]):
                empty_cells.add(cell_map[cell])
                cell_map[cell] = (
                    cell_map[cell][0] + total_extensions[0][pair[1][0] + 1],
                    cell_map[cell][1],
                )
            for cell in param.map.preimage_of_cell(pair[0]):
                empty_cells.add(
                    (
                        cell_map[cell][0] + total_extensions[0][pair[0][0] + 1],
                        cell_map[cell][1],
                    )
                )
        for pair in inequalities[1]:
            for cell in param.map.preimage_of_cell(pair[1]):
                empty_cells.add(cell_map[cell])
                cell_map[cell] = (
                    cell_map[cell][0],
                    cell_map[cell][1] + total_extensions[0][pair[1][1] + 1],
                )
            for cell in param.map.preimage_of_cell(pair[0]):
                empty_cells.add(
                    (
                        cell_map[cell][0],
                        cell_map[1] + total_extensions[0][pair[0][1] + 1],
                    )
                )
        return cell_map, empty_cells
    ...
    def make_new_parameter(self, base_extensions, param: Parameter) -> Parameter:
        ...
        cell_map, empty_cells = self.cell_map_and_empty_cells(total_extensions, param)
        new_obstructions = [
            self.transform_gcp(cell_map, ob) for ob in param.ghost.obstructions
        ] + [
            GriddedCayleyPerm(CayleyPermutation((0,)), (cell,)) for cell in empty_cells
        ]
        new_requirements = [
            [self.transform_gcp(cell_map, req) for req in req_list]
            for req_list in param.ghost.requirements
        ...
        return (
            Parameter(new_ghost, new_map)
            .reduce_empty_rows_and_cols()
            .reduce_by_fusion()
        )
